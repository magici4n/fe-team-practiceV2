...
export default function Home() {
  ...
  return (
    <>
      {/* 실습용 개발 도구. 실제 앱에는 없는 요소라 폰 프레임 바깥에 둡니다 */}
      <div className={`conn-status ${conn}`} role="status">
        {conn === 'loading' && (
          <>
            <span className="dot loading" /> API 서버 연결 확인 중
          </>
        )}
        {conn === 'ok' && (
          <>
            <span className="dot ok" /> API 서버에 연결되었습니다 (localhost:4000)
          </>
        )}
        {conn === 'error' && (
          <>
            <span className="dot error" /> API 서버에 연결할 수 없습니다. 터미널에서{' '}
            <code>npm start</code>를 실행한 뒤 다시 시도해주세요
      ...
      <div className="phone">
        ...
        <header className="app-header">
          <Link to="/" className="brand" aria-label="홈으로 이동">
            ...
            <span className="brand-name">WON 실습뱅킹</span>
          </Link>
          <div className="header-icons">
            <button
              type="button"
              aria-label="알림"
              onClick={() => showToast('알림 화면은 이번 실습 범위 밖입니다')}
            >
            ...
            <button
              type="button"
              aria-label="전체 메뉴 열기"
              onClick={() => showToast('전체 메뉴는 이번 실습 범위 밖입니다')}
            >
              <span aria-hidden="true">☰</span>
            </button>
          </div>
        ...
        <footer>
          <nav aria-label="주요 메뉴" className="bottom-nav">
            <Link to="/" className="nav-btn active" aria-current="page">
              <span className="nav-icon" aria-hidden="true">🏠</span>
              <span className="nav-lbl">홈</span>
            </Link>
            <Link to="/transfer" className="nav-btn">
              <span className="nav-icon" aria-hidden="true">💳</span>
